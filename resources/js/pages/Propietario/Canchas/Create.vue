...
const props = defineProps<{ centros: any[] }>();

const form = useForm({
    centro_deportivo_id: props.centros[0]?.id ?? null,
    nombre: '',
    precio_por_bloque: 0,
    max_bloques_reserva: 1,
    activo: true,
});

function submit() {
    form.post(route('propietario.canchas.store'));
}
</script>
    ...
    <div class="min-h-screen bg-surface px-4 py-8">
        <div class="mx-auto max-w-3xl">
            <div class="rounded-3xl border border-sidebar-border/70 bg-white p-8 shadow-sm">
                <h1 class="text-2xl font-bold">Crear Cancha</h1>
                ...
                <form @submit.prevent="submit" class="mt-6 space-y-4">
                    ...
                    <div class="flex items-center gap-3">
                        <button type="submit" class="bg-primary text-white px-4 py-2 rounded-lg">Crear</button>
